...
const SimpleAuth = () => {
  ...
  const testUsers = [
    { email: 'gjest@leily.no', name: 'Gjest', icon: '👤' },
    { email: 'pro@leily.no', name: 'Pro', icon: '⭐' },
    { email: 'ambassador@leily.no', name: 'Ambassadør', icon: '🏅' }
  ];
      ...
      <div className="w-full max-w-md space-y-6">
        ...
        <Card className="shadow-medium">
          ...
          <CardContent className="space-y-4">
            ...
            {!isProduction && testUsers.map((user) => (
              <Button
                key={user.name}
                variant="outline"
                className="w-full"
            ...
            {/* Test Login Password Input - only in non-production */}
            {!isProduction && showTestLogin && (
